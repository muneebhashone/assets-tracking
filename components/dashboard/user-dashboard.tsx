...
import { UserDashboardStats } from "@/services/companies.queries";
import { StatCard } from "./stat-card";

interface UserDashboardProps {
  data: UserDashboardStats | undefined;
  isLoading: boolean;
}
...
export default function UserDashboard({ data, isLoading }: UserDashboardProps) {
  ...
  const companyGrowthData = data && [
    ...
    { name: "Last 30 Days", value: data.company.newChildCompanies.last30Days },
  ];
    ...
    <div className="space-y-4">
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={data?.shipments.totalShipments ?? 0}
          description={
            data?.shipments.newShipments.today.total
              ? `+${data?.shipments.newShipments.today.total} today`
              : ""
          }
        />
        <StatCard
          title="Child Companies"
          value={data?.company.totalChildCompanies ?? 0}
          description={`${data?.company.activeChildCompanies ?? 0} active`}
        />
        <StatCard
          title="New Companies (30d)"
          value={data?.company.newChildCompanies.last30Days ?? 0}
        />
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="col-span-1">
          ...
          <CardContent className="h-[300px]">
            {Object.keys(data?.shipments?.statusDistribution ?? []).length ===
            0 ? (
              <div className="flex h-full items-center justify-center text-muted-foreground">
                No data available
              </div>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {data?.company.companyDetails?.name ?? "N/A"}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {data?.company.companyDetails?.city ?? "N/A"}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {data.company.companyDetails?.industry ?? "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium">Address</p>
                  <p className="text-sm text-muted-foreground">
                    {data.company.companyDetails?.address ?? "N/A"}
                  </p>
                </div>
              </div>
            )}
